...
%% This script should be used if tracking dots in only a single color


% Initialize Variables
clc;
image_directory = '/Users/Jeff/Desktop/07_01_2017_Batch50/Stacks';            %% Set the image directory with the images with spots to track are
roi_folder = '###ROI_DIRECTORY';                   %% Set the roi directory
...
max_frames = 1000;                                       %% Number of timepoints in movie.
channel = java.lang.Integer(X);                                           %% Which channel to analyze and track.
allow_merging = false;
...
for count = 1:size(dir_images,1)
    ...
    if (match_size >= 1)
        ...
        for k = 1:size(match_roi, 2)
            ...
            if (quality_length(1) > spot_cutoff)   %% If there are more spots found than desired, find the appropriate QUALITY score to filter for the desired number of spots.
