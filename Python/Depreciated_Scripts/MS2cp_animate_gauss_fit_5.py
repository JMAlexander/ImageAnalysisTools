...
def update (frame_number):
    ...
    # Plot image
    roi_1.set_bounds(local_x_min, local_y_min, local_size, local_size);
    ...
    try:
        ...
        data_fitted = data_fitted.reshape(local_x_pixels, local_y_pixels);
        cntr_data = np.full((x_pixels, y_pixels), calc_offset);
        cntr_data[local_y_min:local_y_max, local_x_min:local_x_max] = data_fitted
        cntr = ax.contour(cntr_data, 3, colors='r', linewidths=0.25)
...
with PdfPages('MS2_expression_plots.pdf') as pdf:
    ...
    for i in image_list:
        ...
        for ml in match_list:
            ...
            if (df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (frame_number + 1)),'C1_Y Value (pixel)'].empty and df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (frame_number + 1)),'C1_Y Value (pixel)'].empty):
                for f in range(0, frames):
                    if (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'C1_Y Value (pixel)'].empty) and (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'C1_Y Value (pixel)'].empty):
            ...
            roi_1 = ax.add_patch(patches.Rectangle((local_x_min, local_y_min), local_size, local_size, fill=False, ls='dotted', ec='yellow'));
            # Plot image
            ...
            try:
                ...
                cntr_data = np.full((x_pixels, y_pixels), calc_offset);
                cntr_data[local_y_min:local_y_max, local_x_min:local_x_max] = data_fitted
                cntr = ax.contour(x_global, y_global, cntr_data, 3, colors='r', linewidths=0.25)
            ...
            for f in range(0, frames):
                ...
                if (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'C1_Y Value (pixel)'].empty) and (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'C1_X Value (pixel)'].empty):
                    ...
                    for count in range(0, frames):
                        if (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (count + 1)),'C1_Y Value (pixel)'].empty) and (not df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (count + 1)),'C1_Y Value (pixel)'].empty):
                            locus_loc_y = df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (count + 1)),'C1_Y Value (pixel)'];
                            locus_loc_y = locus_loc_y.values[0];
                            locus_loc_x = df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (count + 1)),'C1_X Value (pixel)'];
                            locus_loc_x = locus_loc_x.values[0];

                            break
            
                if df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),].empty:
                    df = df.append(pd.DataFrame([[ml, np.nan, np.nan, (f + 1), 0, 0, 0, 0]], columns=('C1_Image', 'C1_X Value (pixel)', 'C1_Y Value (pixel)', 'C1_T Value (frame)', 'Gaussian_Height', 'Gaussian_Volume', 'Background', 'Norm_Height')));
                ########
                ## Gaussian Quantitation
                ########
                    if (locus_loc_x - local_halfwidth) < 0:
            ...
            for f in range(0, frames):
                df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'Norm_Height'] = (df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'Gaussian_Height'] + df.loc[(df['C1_Image'] == ml) & (df['C1_T Value (frame)'] == (f + 1)),'Background']) / normalization_factor;
