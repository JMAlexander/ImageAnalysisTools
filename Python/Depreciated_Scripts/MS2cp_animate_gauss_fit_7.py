...
parser.add_argument('-c', '--cutoff', type=float, default=1.5, help='minimum value of Gaussian height required for Gaussian fit.');
...
df = df.assign(Gaussian_Height = 0, Gaussian_Volume = 0, Background = 0, X_Location = 0, Y_Location = 0, X_Sigma = 0, Y_Sigma = 0, Norm_Height = 0, Pass_Filter = True);
pi = 3.14159
local_size = 15;
gauss_halfwidth = 5;
...
output_dir = str(ratio) + "_min_height/"
...
def update (frame_number):
    ...
    for n in range(0, num_loci):
        ...
        bound_tup = ([height_low_bound, local_offset, local_offset, 0, 0, 0], [+np.Inf, (local_size - local_offset), (local_size - local_offset), 5.0, 5.0, +np.Inf]);
        # Plot image
        ...
        try:
            ...
            if (calc_sig_x > 3) or (calc_sig_y > 3) or ((calc_sig_x > 2.0) and (calc_sig_y > 2.0)):
                center_x[n] = np.nan
                center_y[n] = np.nan
        ...
        plt.setp(scat, linewidth=(((calc_amp + calc_offset) / calc_offset)) - 1)
    ...
    for i in image_list:
        ...
        for n in range(0, num_loci):
            ...
            try:
                ...
                signal = 2 * pi * calc_amp * calc_sig_x * calc_sig_y
                ### Plot contour of Gaussian
                if (calc_sig_x > 3.0) or (calc_sig_y > 3.0) or ((calc_sig_x > 2.0) and (calc_sig_y > 2.0)):
                    signal = 0
                    calc_amp = 0
                    center_x[n] = np.nan
                    center_y[n] = np.nan
        ...
        scat = ax.scatter([center_x], [center_y], marker='o', facecolors = 'none', edgecolors='r', s=25, linewidth=(((calc_amp + calc_offset) / calc_offset)) - 1);
        ...
        for l in uloci:
            ...
            for f in range(0, frames):
                ...
                if df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1)),].empty:
                    df = df.append(pd.DataFrame([[l, np.nan, np.nan, (f + 1), 0, 0, 0, 0, 0 ,0, 0, 0, False]], columns=('C1_Image', 'C1_X Value (pixel)', 'C1_Y Value (pixel)', 'C1_T Value (frame)', 'Gaussian_Height', 'Gaussian_Volume', 'Background', 'X_Location', 'Y_Location', 'X_Sigma', 'Y_Sigma', 'Norm_Height', 'Pass_Filter')));
                ########
                ## Gaussian Quantitation
                ########
            ...
            for f in range(0, frames):
                ...
                if (df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1)), 'Pass_Filter'] == True):
                    for loop in range(0,4):
                        if (df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f - loop)), 'Pass_Filter'] == True):
                            noise = False
                        if (df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1 + loop)), 'Pass_Filter'] == True):
                            noise = False
                    if noise == True:
                        df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1)), 'Pass_Filter'] = False
                        df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1)),'Norm_Height'] =  + df.loc[(df['C1_Image'] == l) & (df['C1_T Value (frame)'] == (f + 1)),'Background']) / normalization_factor;
