...
def update (frame_number):
    ...
    if (locus_loc_x - local_halfwidth) < 0:
        local_x_min = 0;
        local_x_max = int(locus_loc_x + (local_halfwidth * 2))
    elif (locus_loc_x + local_halfwidth) > x_size:
        local_x_min = int(locus_loc_x - (local_halfwidth * 2))
        local_x_max = x_size
    else:
    ...
    if (locus_loc_y - local_halfwidth) < 0:
        local_y_min = 0;
        local_y_max = int(locus_loc_x + (local_halfwidth * 2))
    elif (locus_loc_y + local_halfwidth) > y_size:
        local_y_min = int(locus_loc_y - (local_halfwidth * 2))
        local_y_max = y_size
    else:
    ...
    x_guess, y_guess = np.unravel_index(cur_frame_local_data.argmax(), cur_frame_local_data.shape);
    ...
    bound_tup = ([height_low_bound, (local_x_min + local_offset), (local_y_min + local_offset), 0, 0, (off_guess - 4000)], [+np.Inf, (local_x_max - local_offset), (local_y_max - local_offset), 1.6, 1.6, (off_guess + 4000)]);
    # Plot image
    ...
    try:
        ...
        cntr = ax.contour(data_fitted.reshape(x_pixels, y_pixels), 3, colors='r', linewidths=1)
        print(popt[:])
...
with PdfPages('MS2_signal_test.pdf') as pdf:
    ...
    for i in image_list:
        ...
        for ml in match_list:
            ...
            amp_guess = local_max_value - off_guess;
            initial_guess = (amp_guess,x_guess,y_guess,1.0,1.0,off_guess);
            bound_tup = ([height_low_bound, (local_x_min + local_offset), (local_y_min + local_offset), 0, 0, 0], [+np.Inf, (local_x_max - local_offset), (local_y_max - local_offset), 1.6, 1.6, +np.Inf]);
            ...
            try:
                ...
                ### Plot contour of Gaussian
                data_fitted = twoD_Gaussian((x, y), *popt);
                cntr = ax.contour(x, y, data_fitted.reshape(x_pixels, y_pixels), 3, colors='r', linewidths=1.0)
