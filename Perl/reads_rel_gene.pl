...
foreach (@refseq_file){
	...
	$gene_length = abs ($gene_start - $gene_end) + 1;
		...
		foreach (@active_reads){
			...
			elsif ($_ > ($gene_start - 5000) and  ($_ < $gene_start)){
				$rel_pos = ($_ - $gene_start) / 25000;
				$rel_pos = sprintf ("%.4f", $rel_pos);
				$gene_reads .= "$rel_pos ";
			}
			elsif ($_ >=  $gene_start){
				$rel_pos = ($_ - $gene_start) / $gene_length;
				$rel_pos = sprintf ("%.4f", $rel_pos);
				$gene_reads .= "$rel_pos ";
			}
		}
		...
		foreach (@active_reads){
			if ($_ > ($gene_start + 5000)){
				last;
			}
			elsif (($_ > $gene_end) and ($_ <= $gene_start)){
				$rel_pos = ($gene_start - $_) / $gene_length;
				$rel_pos = sprintf ("%.4f", $rel_pos);
				$gene_reads .= "$rel_pos ";
			}
			elsif ($_ > $gene_start){
				$rel_pos = ($gene_start - $_) / 25000;
				$rel_pos = sprintf ("%.4f", $rel_pos);
				$gene_reads .= "$rel_pos ";
			}
		}
