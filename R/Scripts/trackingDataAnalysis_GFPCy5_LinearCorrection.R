...
C3C1_Xcorr <- function(x_pos){
    cor_pos <- 0.0005*x_pos + 0.02553
    return(cor_pos)
}
C3C1_Ycorr <- function(y_pos){
    cor_pos <- -0.00044*y_pos + 0.01949
    return(cor_pos)
}
C3C1_Zcorr <- function(z_pos){
    cor_pos <- -0.00325*z_pos - 0.15869
    return(cor_pos)
}


#x_correction_offset <-  -0.00637;  ##Added to Cy5 channel (Channel1) ##Based on Batch32 data
#y_correction_offset <-  -0.0091;   ##Added to Cy5 channel (Channel1)
#z_correction_offset <-   0.111;    ##Added to Cy5 channel (Channel1)
...
BatchXX_data_c1_spots <- BatchXX_data_c1_spots[!duplicated(BatchXX_data_c1_spots[,c("C1_Image", "C1_T Value (frame)")]),];
...
BatchXX_data_c2_spots <- BatchXX_data_c2_spots[!duplicated(BatchXX_data_c2_spots[,c("C2_Image", "C2_T Value (frame)")]),];
...
BatchXX_data_combined$`C1_Corrected X_Value (um)` <- BatchXX_data_combined$`C1_X Value (um)` - C3C1_Xcorr(BatchXX_data_combined$`C1_X Value (um)`);
BatchXX_data_combined$`C1_Corrected Y_Value (um)` <- BatchXX_data_combined$`C1_Y Value (um)` - C3C1_Ycorr(BatchXX_data_combined$`C1_Y Value (um)`);
BatchXX_data_combined$`C1_Corrected Z_Value (um)` <- BatchXX_data_combined$`C1_Z Value (um)` - C3C1_Zcorr(BatchXX_data_combined$`C1_Z Value (um)`);
  ...
  for (j in start:end) {
    if (nrow(cur_data[cur_data$`C1_T Value (frame)` == (j - 2),]) != 0 && nrow(cur_data[cur_data$`C1_T Value (frame)` == (j - 1),]) != 0 && nrow(cur_data[cur_data$`C1_T Value (frame)` == j,]) != 0){
      ...
      XYZ_angle <- acos(dot_uv/(dist_u * dist_v));
        if (XYZ_angle < 0 || is.na(XYZ_angle)){
...
for (i in cell_lines){
    hist(x[x$Cell_Line == i, "Corrected XY_Distance (um)"], breaks=c(0,seq(0.01,1.0,0.01), 10), xlim=c(0,1), xlab="Corrected XY Distance (um)", ylab="Counts", main=paste("Distribution of XY Distances between Labels for", i, "ESCs", sep=" "));
    hist(x[x$Cell_Line == i, "Corrected XYZ_Distance (um)"], breaks=c(0,seq(0.015,1.5,0.015), 10), xlim=c(0,1.5), xlab="Corrected XYZ Distance (um)", ylab="Counts", main=paste("Distribution of XYZ Distances between Labels for", i, "ESCs", sep=" "));
