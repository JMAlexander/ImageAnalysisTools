...
for (i in cell_lines){
par(mfrow=c(3,2))
y <- x[x$`Pass_Threshold_0_0.26` == TRUE,];
z <- x[x$`Pass_Threshold_0_0.26` == FALSE & x$Pass_Proportional_Threshold_0_0.26 == TRUE,]
images <- unique(x[x$Cell_Line == i, "C1_Image"])
for (k in 1:length(images)){current_line <- unique(x[x$C1_Image == images[k], "Cell_Line"]);
plot(x[x$C1_Image == images[k], "C1_T Value (sec)"], x[x$C1_Image == images[k], "Corrected XY_Distance (um)"], type="p", pch=20, main="XY Distance vs. Time with 100nm Threshold Highlighted", sub=paste(current_line, images[k]), ylab="XY Distance (um)", xlab="Time (sec)", ylim=c(0,1.5), xlim=c(0,t_point_step * 100));
lines(x[x$C1_Image == images[k], "C1_T Value (sec)"], abs(x[x$C1_Image == images[k], "Corrected Z_Distance (um)"]), col="yellow");
points(y[y$C1_Image == images[k], "C1_T Value (sec)"], y[y$C1_Image == images[k], "Corrected XY_Distance (um)"], type="p", pch=20, col="orange");
points(z[z$C1_Image == images[k], "C1_T Value (sec)"], z[z$C1_Image == images[k], "Corrected XY_Distance (um)"], type="p", pch=20, col="red");
